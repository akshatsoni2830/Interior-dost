/**
 * Design Page - Interior-Dost MVP
 * 
 * Orchestrates the full pipeline:
 * 1. Image upload (with camera support)
 * 2. Room analysis
...
export default function DesignPage() {
  ...
  const handleRedesign = async () => {
    ...
    if (isAnalyzing || isGenerating || isDetectingFurniture) {
      return;
    }
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12 max-w-7xl">
        ...
        <header className="text-center mb-8 sm:mb-12 animate-fade-in">
          <div className="flex items-center justify-between mb-4">
            <Link
              href="/"
              className="text-amber-700 hover:text-amber-800 font-medium text-sm sm:text-base flex items-center gap-2 transition-colors duration-200"
            >
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-amber-800 via-orange-700 to-amber-900 mb-3 sm:mb-4 leading-tight">
            Interior-Dost
          </h1>
          <p className="text-base sm:text-lg md:text-xl text-stone-700 max-w-2xl mx-auto leading-relaxed px-4">
        ...
        <div className="space-y-6 sm:space-y-8">
          {/* Upload Section */}
          <section 
            className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-4 sm:p-6 md:p-8 transition-all duration-300 hover:shadow-2xl border border-stone-200"
            aria-labelledby="upload-heading"
          >
            <h2 
              id="upload-heading"
              className="text-xl sm:text-2xl md:text-3xl font-bold text-stone-900 mb-4 sm:mb-6 flex items-center gap-2 sm:gap-3"
            >
              <span className="flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-amber-100 text-amber-700 font-bold text-base sm:text-lg flex-shrink-0">
                1
              </span>
              <span className="leading-tight">Upload or Capture Your Room</span>
            </h2>
            <UploadComponent
          ...
          {uploadedImageUrl && (
            <section 
              className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-4 sm:p-6 md:p-8 transition-all duration-300 hover:shadow-2xl animate-slide-in border border-stone-200"
          ...
          {uploadedImageUrl && (
            <div className="flex justify-center animate-slide-in">
              <button
                ...
                disabled={isLoading}
                className="w-full sm:w-auto px-8 sm:px-10 py-4 sm:py-5 bg-gradient-to-r from-amber-600 to-orange-600 text-white text-base sm:text-lg md:text-xl font-bold rounded-xl hover:from-amber-700 hover:to-orange-700 disabled:from-gray-400 disabled:to-gray-500 disabled:cursor-not-allowed transition-all duration-300 shadow-lg hover:shadow-2xl transform hover:scale-105 disabled:transform-none focus:outline-none focus:ring-4 focus:ring-amber-300"
                ...
                {isLoading ? (
                  <span className="flex items-center justify-center gap-3">
                    ...
                    <span>
                      ...
                      {isGenerating && 'Generating redesign... (30-60s)'}
        ...
        {/* Footer */}
        <footer className="mt-12 sm:mt-16 text-center text-stone-600 text-xs sm:text-sm md:text-base space-y-2 px-4">
